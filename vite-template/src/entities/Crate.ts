...
export default class Crate extends Phaser.GameObjects.Sprite {
  ...
  constructor(
    ...
    // Select the correct frame based on size
    const frame =
      size === "big" ? ASSETS.CRATE.BIG.FRAME : ASSETS.CRATE.SMALL.FRAME;

    super(scene, x, y, ASSETS.ATLAS, frame);
    this.scene = scene;
    this.size = size;
  ...
  initPhysics() {
    ...
    const shapeDef = {
      ...
      density:
        this.size === "big"
          ? PHYSICS.CRATE.BIG_DENSITY
          : PHYSICS.CRATE.SMALL_DENSITY,
      friction: PHYSICS.CRATE.FRICTION,
      restitution: PHYSICS.CRATE.RESTITUTION,
      userData: { type: "crate", crateInstance: this, size: this.size },
    };
